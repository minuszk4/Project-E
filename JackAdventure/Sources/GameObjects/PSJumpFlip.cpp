...
{
	m_Animation = new Animation(*DATA->getTexture("Character/Jumpflip"), sf::Vector2i(2, 1), 0.1f);
...
void PSJumpFlip::Update(float deltaTime)
{
	...
	else m_Player->getHitBox()->move(0, m_Player->getHitBox()->getVelocity().y * 0.5 * deltaTime - v);

	if (sf::Keyboard::isKeyPressed(sf::Keyboard::A)) {
		m_Player->getHitBox()->move(-m_Player->getHitBox()->getVelocity().x * 0.5 * deltaTime, 0);
	}
